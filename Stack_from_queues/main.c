...
int listQueueIsEmpty(struct Queue*);
...
void listQueueAddBack (struct Queue* queue, TYPE value){
    ...
    struct Link *currentLast = queue->tail;
    newLink->value = value;
    newLink->next = currentLast->next;
    
    currentLast->next = newLink;
    queue->tail = newLink;
}
...
TYPE listQueueRemoveFront(struct Queue* queue){
    assert(queue != NULL);
    assert(!listQueueIsEmpty(queue));
    struct Link *currFront = queue->head->next;
    queue->head->next = currFront->next;
    if(queue->tail == currFront) queue->tail = queue->head;
    TYPE value = currFront->value;
    free(currFront);
    return value;
}
...
int listStackIsEmpty(struct Stack* stack){
    assert(stack != NULL);
    if(listQueueIsEmpty(stack->q1)) return 1;
    else return 0;
}
...
/**
 ...
 it's empty q1 and q2 are swapped
 */
void listStackPush(struct Stack* stack, TYPE value){
    assert(stack != NULL);
    struct Queue *q2 = stack->q2;
    listQueueAddBack(q2, value);
    while(!listQueueIsEmpty(stack->q1)){
        TYPE value = listQueueRemoveFront(stack->q1);
